...
namespace tlu {
    ...
    if (resbit.test(7))
      {
        ...
        std::printf( "The Enclustra I2C bus is not correctly enabled.");
      }else{
  ...
  void AidaTluController::InitializeDAC(bool intRef, float Vref, uint8_t verbose) {
    m_zeDAC1->SetI2CPar(m_i2c.get(), m_I2C_address.DAC1);
    m_zeDAC1->SetIntRef(intRef, verbose);
    m_zeDAC2->SetI2CPar(m_i2c.get(), m_I2C_address.DAC2);
    m_zeDAC2->SetIntRef(intRef, verbose);
    SetDACref(Vref, verbose);
  }

  void AidaTluController::InitializeIOexp(uint8_t verbose){
    m_IOexpander1->SetI2CPar(m_i2c.get(), m_I2C_address.expander1);
    m_IOexpander2->SetI2CPar(m_i2c.get(), m_I2C_address.expander2);
  ...
  void AidaTluController::InitializeI2C(uint8_t verbose) {
    ...
    if (m_hasDisplay){
      std::printf("AIDA TLU: LCD display detected. This is a 19-inch rack unit.\t");
      m_lcddisp->setParameters(m_i2c.get(), m_I2C_address.lcdDisp, 2, 16);
  ...
  void AidaTluController::pwrled_Initialize(uint8_t verbose, unsigned int type) {
    ...
    m_pwrled->setI2CPar( m_i2c.get() , m_I2C_address.pwraddr, m_I2C_address.ledxp1addr, m_I2C_address.ledxp2addr, type, verbose);
  ...
  //  On start, the internal registers are reset and the TLU issues a T0 pulse to DUTs.
    ...
    std::printf("TLU SET TO %s",  runState.c_str());
  }
  ...
  void AidaTluController::SetDUTMask(int32_t value, uint8_t verbose){
    uint32_t res;
    if (value < 0){
      std::printf("DUTMask can  not be a negavite number.");
      return;
    }
  ...
  void AidaTluController::SetDUTMaskMode(int32_t value, uint8_t verbose){
    uint32_t res;
    if (value < 0){
      std::printf( "DUTMaskMode can  not be a negavite number.");
      return;
    }
  ...
  void AidaTluController::SetDUTMaskModeModifier(int32_t value, uint8_t verbose){
    uint32_t res;
    if (value < 0){
      std::printf("DUTMaskModeModifier can  not be a negavite number.");
      return;
    }
  ...
  void AidaTluController::SetDUTIgnoreBusy(int32_t value, uint8_t verbose){
    uint32_t res;
    if (value < 0){
      std::printf("DUTIgnoreBusy can  not be a negavite number.");
      return;
    }
  ...
  void AidaTluController::SetShutterControl(int32_t value, uint8_t verbose){
    uint32_t res;
    if (value < 0){
      std::printf("ShutterControl can  not be a negavite number.");
      return;
    }
    if (value & 0x1){
      std::printf("TLU is configured to use shutter.\t");
    }

    if (value > 3){
      std::printf("Shutter control: only bit 0 and 1 are considered for this value.\t");
      value= value & 0x3;
    }
  ...
  void AidaTluController::SetShutterInternalInterval(int32_t value, uint8_t verbose){
    uint32_t res;
    if (value < 0){
      std::printf("ShutterInternalInterval can  not be a negavite number.");
      return;
    }
  ...
  void AidaTluController::SetShutterSource(int32_t value, uint8_t verbose){
    ...
    uint32_t res;
    if ((value < 0) | (value > 5)){
      std::printf("Shutter source can only be in range [0, 5]. Value %i ignored.\t",  + value);
      return;
    }
    else{
  ...
  void AidaTluController::SetShutterOnTime(int32_t value, uint8_t verbose){
    uint32_t res;
    if (value < 0){
      std::printf("ShutterOnTime can  not be a negavite number.");
      return;
    }
  ...
  void AidaTluController::SetShutterOffTime(int32_t value, uint8_t verbose){
    uint32_t res;
    if (value < 0){
      std::printf("ShutterOffTime can  not be a negavite number.");
      return;
    }
  ...
  void AidaTluController::SetShutterVetoOffTime(int32_t value, uint8_t verbose){
    uint32_t res;
    if (value < 0){
      std::printf("ShutterVetoOffTime can  not be a negavite number.");
      return;
    }
  ...
  void AidaTluController::SetDUTIgnoreShutterVeto(int32_t value, uint8_t verbose){
    uint32_t res;
    if (value < 0){
      std::printf("DUTIgnoreShutterVeto can  not be a negavite number.");
      return;
    }
  ...
  void AidaTluController::SetShutterParameters(bool status, int8_t source, int32_t onTime, int32_t offTime, int32_t vetoOffTime, int32_t intInterval, uint8_t verbose){
    ...
    if (status){
      ...
      if (vetoOffTime < onTime){
        std::printf("ShutterVetoOffTime can not be shorter than ShutterOnTime. Please review configuration parameters.\t");
        std::cout << "\tENSURE THAT ShutterOnTime <= ShutterVetoOffTime < ShutterOffTime" << std::endl;
        return;
      }
      if (offTime <= onTime){
        std::printf("ShutterOffTime must be longer than ShutterOnTime. Please review configuration parameters.\t");
        std::cout << "\tENSURE THAT ShutterOnTime <= ShutterVetoOffTime < ShutterOffTime" << std::endl;
        return;
      }
      if (offTime <= vetoOffTime){
        std::printf("ShutterOffTime must be longer than ShutterVetoOffTime. Please review configuration parameters.\t");
        std::cout << "\tENSURE THAT ShutterOnTime <= ShutterVetoOffTime < ShutterOffTime" << std::endl;
        return;
      }
  ...
  void AidaTluController::SetInternalTriggerFrequency(int32_t user_freq, uint8_t verbose){
    ...
    if (user_freq < 0){
      std::printf("InternalTriggerFrequency can  not be a negavite number.");
      return;
    }
